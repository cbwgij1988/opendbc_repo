...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret: structs.CarParams, candidate, fingerprint, car_fw, alpha_long, is_release, docs) -> structs.CarParams:
    ...
    if candidate in TSS2_CAR:
      ...
      ret.deprecated.vEgoStopping = 0.25 if sp_tss2_long_tune else 0.25
      ret.deprecated.vEgoStarting = 0.01 if sp_tss2_long_tune else 0.25
      if candidate == CAR.TOYOTA_RAV4_TSS2:
        ret.deprecated.stoppingDecelRate = 0.07 if sp_tss2_long_tune else 0.3   # reach stopping target smoothly
      else:
        ret.deprecated.stoppingDecelRate = 0.07 if sp_tss2_long_tune else 0.3  # reach stopping target smoothly
